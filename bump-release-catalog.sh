...
hash=$(nix-hash --type sha256 --base32 --flat <(
  nix-shell --pure -E 'with import ./pkgs {}; buildEnv { name = "dump-catalogs"; paths = []; buildInputs = [ racket cacert ]; }' --run "LANG=C LC_ALL=C racket -U -N dump-catalogs nix/dump-catalogs.rkt $url"))
